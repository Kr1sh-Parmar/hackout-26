...
ZONES = {"DE": "DE", "DE_50hertz": "DE-50HERTZ", "DE_amprion": "DE-AMPRION",
         "DE_tennet": "DE-TENNET", "DE_transnetbw": "DE-TRANSNETBW"}
...
def build(freq: str) -> None:
    ...
    cols = ["utc_timestamp"] + [c for c in d.columns
                                if c.startswith(keep_prefix) and c != "utc_timestamp"]
    d = d[cols]
    ...
    for zone, region_id in ZONES.items():
        for tech, gen, cap in [
            ...
            ("wind_onshore", f"{zone}_wind_onshore_generation_actual",
             f"{zone}_wind_onshore_capacity"),
            ("wind_offshore", f"{zone}_wind_offshore_generation_actual",
             f"{zone}_wind_offshore_capacity"),
        ]:
            if gen not in d.columns:
                continue
            r = pd.DataFrame({
                "region_id": region_id, "ts_utc": d.ts_utc, "tech": tech,
                "power_mw": d[gen],
    ...
    ld = []
    for zone, region_id in ZONES.items():
        a, fc = (f"{zone}_load_actual_entsoe_transparency",
                 f"{zone}_load_forecast_entsoe_transparency")
        if a not in d.columns:
            continue
        ld.append(pd.DataFrame({
            "region_id": region_id, "ts_utc": d.ts_utc,
            "demand_mw": d[a],
