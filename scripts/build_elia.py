...
def _write(df: pd.DataFrame, layer: pathlib.Path, name: str) -> None:
    ...
    print(
        f"  wrote {layer.name}/{name:26} {len(df):>9,} rows  {df.memory_usage(deep=True).sum() / 1e6:>6.1f} MB"
    )
...
def qc_flag(power: pd.Series, cap: pd.Series, curtailed: pd.Series) -> pd.Series:
    ...
    stage
    here we flag only what is determinable from the source itself."""
...
def build_wind() -> None:
    ...
    seg = (
        df.groupby(["ts_utc", "offshoreonshore"], observed=True)
        .agg(
            power_mw=("measured", "sum"),
    ...
    print(f"  wind: expected {expected} segments/interval; {partial.sum():,} intervals are partial")
    ...
    out = pd.DataFrame(
        {
            ...
            "ts_utc": nat.ts_utc,
            "tech": "wind",
            "power_mw": nat.power_mw,
            "monitored_capacity_mw": nat.monitored_capacity_mw,
...
def _tso(src: pd.DataFrame, tech: str) -> pd.DataFrame:
    ...
    benchmark
    keeping the vintages separate is what lets us match the cutoff."""
    rows = []
    for horizon, (p50, p10, p90) in HORIZONS.items():
        r = pd.DataFrame(
            {
                "region_id": REGION_ID,
                "ts_utc": src.ts_utc.values,
                "tech": tech,
                "horizon": horizon,
                "p10_mw": src[p10].values,
                "p50_mw": src[p50].values,
                "p90_mw": src[p90].values,
            }
        )
        rows.append(r)
