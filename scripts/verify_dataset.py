...
def main() -> int:
    ...
    check(
        "wind in m/s not km/h (max 100m < 60)",
        wx.wind_speed_100m_ms.max() < 60,
        f"max={wx.wind_speed_100m_ms.max():.1f} m/s",
    )
    check(
        "irradiance in physical range (< 1400 W/m2)",
        wx.ghi_wm2.max() < 1400,
        f"max={wx.ghi_wm2.max():.0f} W/m2",
    )
    check(
        "GHI plausible for Belgium (700-1000 W/m2 peak)",
        700 <= wx.ghi_wm2.max() <= 1100,
        f"max={wx.ghi_wm2.max():.0f}",
    )
    check(
    ...
    check(
        "weather PK unique (run,valid,model,point)",
    ...
    check(
        "all 3 NWP models present at every grid point",
        (per > 0).all().all(),
        f"{per.shape[0]} points x {per.shape[1]} models",
    )
    check(
        "grid point weights sum to 1",
    ...
    cov = (
        wx.groupby(["forecast_vintage", "nwp_model"])["grid_point_id"]
        .nunique()
    ...
    print(cov.to_string())
    check(
        "every vintage x model covers all grid points",
        worst == n_points,
        f"min {worst}/{n_points} points",
    )
    wsum = (
        wx.drop_duplicates(["forecast_vintage", "nwp_model", "grid_point_id"])
        .groupby(["forecast_vintage", "nwp_model"])
        .weight.sum()
    )
    check(
        "capacity weight covered >= 0.99 for every vintage x model",
        wsum.min() > 0.99,
        f"min weight covered={wsum.min():.2f}",
    )
    ...
    if len(tr):
        check(
            "lead_hours covers the 24-72 h horizon",
            tr.lead_hours.min() <= 24 and tr.lead_hours.max() >= 71,
            f"{tr.lead_hours.min()}..{tr.lead_hours.max()} h",
        )
        check("real (non-approximated) run_ts on training vintages", (~tr.run_ts_is_approx).all())
    ...
    for nm, d in (("solar", sol), ("wind", wnd)):
        ...
        check(f"{nm} power <= monitored capacity (+5% slack)", ok, f"max load factor={worst:.3f}")
        ...
        check(
            f"{nm} negatives are flagged and small",
            neg.empty
            or (neg.qc_flag.eq("OUT_OF_RANGE").all() and neg.power_mw.min() > -0.01 * cap),
        ...
        check(f"{nm} mostly OK (<5% flagged)", frac.get("OK", 0) > 95, str(frac))
    ...
    check(
        "weather/generation/load overlap > 1 year",
        (ov_hi - ov_lo).days > 365,
        f"{ov_lo.date()} -> {ov_hi.date()} ({(ov_hi - ov_lo).days} days)",
    )
    if len(tr):
        t_lo = max(tr.valid_ts_utc.min(), sol.ts_utc.min())
        t_hi = min(tr.valid_ts_utc.max(), sol.ts_utc.max())
        check(
            "24-72h trainable overlap > 365 days",
            (t_hi - t_lo).days > 365,
            f"{t_lo.date()} -> {t_hi.date()} ({(t_hi - t_lo).days} days)",
        )
    ...
    for nm, d in (("solar", sol), ("wind", wnd), ("load", load)):
        ...
        check(
            f"{nm} interval completeness",
            len(missing) / len(full) < 0.01,
    ...
    print("== TSO benchmark ==")
    check(
        "Elia forecast vintages present",
    ...
    check(
        "Elia day-ahead solar nRMSE is a sane benchmark",
        0.01 < nrmse < 0.20,
        f"nRMSE={nrmse * 100:.2f}% of capacity over {len(day):,} daylight intervals",
    )
    ...
    if gp.exists():
        ...
        check(
            "NWP disagreement grows with lead time",
            far > near,
            f"lead<=47h: {near:.1f} W/m2 vs lead>47h: {far:.1f} W/m2",
        )
        check(
            "no lead hour below 24 in the training set",
            g.lead_hours.min() >= 24,
            f"min={g.lead_hours.min()} h",
        )
        check(
            "target is capacity factor in [0, 1.05]",
    ...
    print(
        f"{len(RESULTS)} checks: {len(RESULTS) - n_fail - n_warn} pass, {n_warn} warn, {n_fail} FAIL"
    )
