...
ZEN = pathlib.Path("data/raw/india"); ZEN.mkdir(parents=True, exist_ok=True)
OPSD = pathlib.Path("data/raw/opsd"); OPSD.mkdir(parents=True, exist_ok=True)
    ...
    for attempt in (1, 2, 3):
        try:
            with requests.get(url, stream=True, timeout=900,
                              headers={"User-Agent": "zero-bias-research/1.0"}) as r:
                r.raise_for_status()
